...
function renderChainBlock(els, y, item, role, isUp) {
  ...
  if (pics.length) {
    ...
    if (pics.length === 1) {
      const iw = Math.min(320, maxImgW), ih = Math.round(iw * 0.75);
      imgH = ih;
      imgBlock += `<image href="${picImgs[0] || ''}" x="${nameX}" y="${imgY}" width="${iw}" height="${ih}" clip-path="url(#imgClipBig)" preserveAspectRatio="xMidYMid slice"/>`;
      if (!picImgs[0]) imgBlock += `<rect x="${nameX}" y="${imgY}" width="${iw}" height="${ih}" rx="10" fill="rgba(255,255,255,0.06)"/>`;
    } else {
