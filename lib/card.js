...
/** 下载原始图片 buffer（带缓存；失败返回 null） */
function fetchBuf(url) {
  if (!url) return Promise.resolve(null);
  if (bufCache.has(url)) return bufCache.get(url);
  const p = downloadImage(url).then(b => { if (!b) return null; return b; });
  bufCache.set(url, p);
  return p;
}
...
async function buildSvg(comment, replies, opts = {}) {
  ...
  // 2. 主评论卡片
  const mc = renderMainCard(comment, y);
  els.push(...mc.els);
  y = mc.cardBottom;
...
function renderMainCard(comment, startY) {
  const els = [];
  let y = startY;
  const cardTop = y;
  ...
  if (comment.pictures && comment.pictures.length) {
    ...
    if (pics.length === 1) {
      ...
      els.push(`<image href="${picImgs[0] || ''}" x="${INNER_X}" y="${y}" width="${f.w}" height="${f.h}" clip-path="url(#imgClipBig)" preserveAspectRatio="xMidYMid slice"/>`);
...
function renderChainBlock(els, y, item, role, isUp) {
  ...
  if (pics.length) {
    ...
    if (pics.length === 1) {
      ...
      imgH = f.h;
      imgBlock += `<image href="${picImgs[0] || ''}" x="${nameX}" y="${imgY}" width="${f.w}" height="${f.h}" clip-path="url(#imgClipBig)" preserveAspectRatio="xMidYMid slice"/>`;
      ...
      pics.forEach((_, i) => {
        ...
        imgBlock += `<image href="${picImgs[i] || ''}" x="${cx}" y="${cy}" width="${cell}" height="${cell}" clip-path="url(#imgClip)" preserveAspectRatio="xMidYMid slice"/>`;
