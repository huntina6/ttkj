...
/** 分页拉取全部子回复（B站每页最多返回 20 条；不满 20 视为最后一页） */
async function getAllSubReplies(oid, type, rootRpid, cookie, maxPages = 5) {
  const all = [];
  for (let page = 1; page <= maxPages; page++) {
    const data = await apiGet(`/x/v2/reply/reply?type=${type}&oid=${oid}&root=${rootRpid}&pn=${page}&ps=50`, {
      cookie,
      referer: `https://t.bilibili.com/${oid}`,
    });
...
function filterUpInteractions(replies, uid) {
  ...
  const upReplies = replies.filter(r => r.mid === uid);
  ...
  for (const lk of upLiked) {
    if (lk.mid === uid) continue; // UP 自己的回复不重复算点赞
