...
### 修复（2026-08-15）
- UP 互动识别失效（B站 API mid 为数字、CLI uid 为字符串，严格比较恒 false）→ 互动回顾图/UP 标识恢复正常
- 主卡片无条件显示「UP主」徽标（粉丝评论误标）→ 按评论作者是否为目标 UP 条件显示
- 交互模式粘贴 t.bilibili.com 链接解析失败 → 统一 extractId 解析
- 置顶评论被删除（-404）时程序报错 → 优雅降级为「无置顶评论」
- export-svg.js 头像/表情丢失；--rpid 缺 --oid 无限报错；--type 非法值；图片下载失败缓存
- 终端横幅（BANNER）歪斜：CJK 全角字符按 2 列显示宽度动态对齐填充空格，边框左右字符独立
- 交互模式 Cookie 提示误导：有已保存 Cookie 时明确提示（回车沿用，输入 clear 清除后匿名）
- 选择器结束后程序退出的 Bug（stdin.pause() 阻断后续 readline 输入）→ 交互链恢复流畅

### 增强（2026-08-15）
- 交互模式全新 UI：分组分区（运行模式/目标设置/监控行为/卡片与输出）、➤ 提示符、配置完成汇总
- 运行模式方向键选择器：↑/↓ 移动高亮光标、回车确认（支持循环与回绕）
- 是/否提问横排开关选择器：←/→（或 ↑/↓、空格）切换、回车确认，兼容 y/n 按键
- 高亮箭头（❯/➤）统一为粉色加粗，与品牌色一致
